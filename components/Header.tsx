...
type HeaderType = "home" | "notifications" | "profile";

const Header: FC<{ type: HeaderType; name?: string; tweetCount?: number }> = ({
    type,
    name,
    tweetCount,
}) => {
    ...
    switch (type) {
        ...
        case "profile":
            return (
                <div className="bg-theme-blur sticky top-0 z-50 flex h-[50px] w-full items-center p-[15px]">
                    <div
                        className="-ml-[6.5px] mr-[27px] cursor-pointer rounded-full p-[6.5px] light:hover:bg-[#e6e7e7] dim:hover:bg-[#2c3640] dark:hover:bg-[#181818]"
                        onClick={() => router.back()}
                    >
                        <svg
                            viewBox="0 0 24 24"
                            className="w-[19px] max-w-full fill-[#e7e9ea] light:fill-[#0f1419]"
                        >
                            <g>
